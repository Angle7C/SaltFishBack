...
public class ExecptionHandler {
    private  static  final Logger logger= LoggerFactory.getLogger("异常抓取");
    @ExceptionHandler(Throwable.class)
    public ResultJson unkownHandle(Throwable throwable){
        ResultJson json=new ResultJson();
        logger.error("出现异常：{} 异常消息；{} 异常地点：{}",throwable.getMessage(),throwable.getCause(),throwable.getStackTrace()[0]);
...
//        ResultJson json=new ResultJson();
    ...
    public ResultJson mailHandle(MailException e){
        ResultJson json=new ResultJson();
        logger.error("邮箱异常：{},出现异常:{}",e,e.getMessage());
